...
class TestGameE2E:
    ...
    @pytest.mark.asyncio
    async def test_two_player_game_to_completion(self, client: AsyncClient):
        ...
        start_response = await client.post(f"/api/games/{game_id}/start?player_id={player1_id}")
        ...
        while rounds_played < max_rounds:
            ...
            advance_response = await client.post(f"/api/games/{game_id}/advance?player_id={player1_id}")
    ...
    @pytest.mark.asyncio
    async def test_judge_rotation(self, client: AsyncClient):
        """Test that the judge rotates between players each round."""
        # Create game and add player
        create_resp = await client.post("/api/games", json={"host_nickname": "Alice"})
        game_id = create_resp.json()["game_id"]
        invite_code = create_resp.json()["invite_code"]
        player1_id = create_resp.json()["player_id"]

        join_resp = await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Bob"})
    ...
    @pytest.mark.asyncio
    async def test_tiles_replenished_after_round(self, client: AsyncClient):
        ...
        state_after_submit_resp = await client.get(f"/api/games/{game_id}?player_id={player1_id}")
        state_after_submit = state_after_submit_resp.json()
        ...
        state_after_round_resp = await client.get(f"/api/games/{game_id}?player_id={player1_id}")
        state_after_round = state_after_round_resp.json()

        if state_after_round["phase"] != "game_over":
            assert len(state_after_round["my_tiles"]) == tiles_per_player

    @pytest.mark.asyncio
    async def test_only_host_can_start_and_advance(self, client: AsyncClient):
        ...
        start_resp = await client.post(f"/api/games/{game_id}/start?player_id={player2_id}")
        assert start_resp.status_code == 403

        # Host starts successfully
        start_resp = await client.post(f"/api/games/{game_id}/start?player_id={player1_id}")
        ...
        advance_resp = await client.post(f"/api/games/{game_id}/advance?player_id={player2_id}")
        assert advance_resp.status_code == 403

        # Host advances successfully
        advance_resp = await client.post(f"/api/games/{game_id}/advance?player_id={player1_id}")
        assert advance_resp.status_code == 200

    @pytest.mark.asyncio
    async def test_only_judge_can_select_winner(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_three_player_game_with_multiple_submissions(self, client: AsyncClient):
        ...
        join_resp2 = await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Bob"})
        player2_id = join_resp2.json()["player_id"]

        join_resp3 = await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Charlie"})
        ...
        for player_id in all_player_ids:
            player_state_resp = await client.get(f"/api/games/{game_id}?player_id={player_id}")
            player_state = player_state_resp.json()
    ...
    @pytest.mark.asyncio
    async def test_cannot_join_game_after_start(self, client: AsyncClient):
        ...
        await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Bob"})

        # Start the game
        await client.post(f"/api/games/{game_id}/start?player_id={player1_id}")

        # Try to join after start - should fail
        join_resp = await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Charlie"})
        assert join_resp.status_code == 400
        assert "lobby" in join_resp.json()["detail"]["error"].lower()

    @pytest.mark.asyncio
    async def test_all_players_can_submit(self, client: AsyncClient):
        ...
        submit_resp1 = await client.post(
            f"/api/games/{game_id}/submit?player_id={player1_id}",
            json={"tiles_used": state1["my_tiles"][:1]},
        )
    ...
    @pytest.mark.asyncio
    async def test_player_cannot_submit_twice(self, client: AsyncClient):
        ...
        await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Bob"})
        await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Charlie"})
    ...
    @pytest.mark.asyncio
    async def test_cannot_start_with_insufficient_players(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_game_full_prevention(self, client: AsyncClient):
        ...
        join_resp2 = await client.post(f"/api/games/join/{invite_code}", json={"nickname": "Charlie"})
    ...
    @pytest.mark.asyncio
    async def test_cannot_submit_tiles_player_does_not_have(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_custom_game_configuration(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_cannot_select_nonexistent_player_as_winner(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_game_not_found_errors(self, client: AsyncClient):
        ...
        # Get game state
        resp = await client.get(f"/api/games/{fake_game_id}?player_id={fake_player_id}")
        assert resp.status_code == 404

        # Start game
        resp = await client.post(f"/api/games/{fake_game_id}/start?player_id={fake_player_id}")
        assert resp.status_code == 404
        ...
        resp = await client.post("/api/games/join/INVALID", json={"nickname": "Test"})
        assert resp.status_code == 404

    @pytest.mark.asyncio
    async def test_error_responses_contain_helpful_info(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_player_not_found_in_game_error(self, client: AsyncClient):
        ...
        resp = await client.get(f"/api/games/{game_id}?player_id=invalid-player-id")
        assert resp.status_code == 404
        error_detail = resp.json()["detail"]
        assert error_detail["code"] == "PLAYER_NOT_FOUND"

    @pytest.mark.asyncio
    async def test_wrong_phase_errors(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_judge_picks_self_triggers_overrule_voting(self, client: AsyncClient):
        ...
        for pid in all_players:
            state_resp = await client.get(f"/api/games/{game_id}?player_id={pid}")
            state = state_resp.json()
            await client.post(
    ...
    @pytest.mark.asyncio
    async def test_unanimous_overrule_reverts_point(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_non_unanimous_overrule_keeps_winner(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_winner_voting_after_overrule(self, client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_two_player_game_no_overrule(self, client: AsyncClient):
